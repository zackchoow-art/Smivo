...
class EvidencePhotoSection extends ConsumerWidget {
        ...
        evidenceAsync.when(
          ...
          data: (photos) {
            final filteredPhotos =
                photos.where((p) => p.evidenceType == evidenceType).toList();
                      ...
                      child: SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          label: Text(
                            uploadState.isLoading
                                ? 'Uploading...'
                                : 'Add Photo (${filteredPhotos.length}/5)',
                          ),
                        ),
                      ),
                    ),
  ...
  Future<void> _pickAndUpload(BuildContext context, WidgetRef ref) async {
    ...
    final fileName =
        '${evidenceType}_${DateTime.now().millisecondsSinceEpoch}.jpg';
    await ref
        .read(evidenceUploaderProvider.notifier)
        .upload(
          orderId: orderId,
          imageBytes: bytes,
          fileName: fileName,
