...
class RentalOptionsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if ((listing.rentalDailyPrice ?? 0) > 0) {
      pricingCards.add(_PricingCard(label: 'DAY', price: listing.rentalDailyPrice!, isSelected: selectedRate == 'DAY',
        onTap: () => ref.read(selectedRentalRateProvider.notifier).setRate('DAY')));
    }
    if ((listing.rentalWeeklyPrice ?? 0) > 0) {
      pricingCards.add(_PricingCard(label: 'WEEK', price: listing.rentalWeeklyPrice!, isSelected: selectedRate == 'WEEK',
        onTap: () => ref.read(selectedRentalRateProvider.notifier).setRate('WEEK')));
    }
    if ((listing.rentalMonthlyPrice ?? 0) > 0) {
      pricingCards.add(_PricingCard(label: 'MONTH', price: listing.rentalMonthlyPrice!, isSelected: selectedRate == 'MONTH',
...
class _PricingCard extends StatelessWidget {
  const _PricingCard({required this.label, required this.price, required this.isSelected, required this.onTap});
  final String label;
  final double price;
  final bool isSelected;
  final VoidCallback onTap;

  String _formatPrice(double p) => p == p.toInt() ? p.toStringAsFixed(0) : p.toStringAsFixed(2);
    ...
    return Expanded(child: GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? colors.priceAccent : colors.surfaceContainerLowest,
          borderRadius: BorderRadius.circular(radius.md),
          border: Border.all(color: isSelected ? colors.priceAccent : colors.outlineVariant.withValues(alpha: 0.3)),
        ),
        child: Column(children: [
          Text(label, style: typo.labelSmall.copyWith(color: isSelected ? Colors.white70 : colors.onSurface.withValues(alpha: 0.5))),
          const SizedBox(height: 4),
          Text('\$${_formatPrice(price)}', style: typo.titleMedium.copyWith(color: isSelected ? Colors.white : colors.priceAccent)),
        ]),
      ),
    ));
  }
...
class _DateRangePicker extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('START DATE', style: typo.labelSmall.copyWith(color: colors.onSurface.withValues(alpha: 0.5))),
          const SizedBox(height: 4),
          _DateBox(text: formatter.format(startDate), onTap: () async {
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('END DATE', style: typo.labelSmall.copyWith(color: colors.onSurface.withValues(alpha: 0.5))),
          const SizedBox(height: 4),
          _DateBox(text: formatter.format(endDate), onTap: () async {
            final picked = await showDatePicker(context: context,
              initialDate: endDate.isBefore(startDate.add(const Duration(days: 1))) ? startDate.add(const Duration(days: 1)) : endDate,
              firstDate: startDate.add(const Duration(days: 1)), lastDate: startDate.add(const Duration(days: 365)));
            if (picked != null) ref.read(rentalEndDateProvider.notifier).setDate(DateTime(picked.year, picked.month, picked.day));
          }),
        ])),
      ]),
      const SizedBox(height: 12),
      _TotalRentBanner(periodText: '${durationDays > 0 ? durationDays : 0} days', totalAmount: totalRent),
    ]);
  }
}
...
class _TotalRentBanner extends StatelessWidget {
  ...
  String _formatTotal(double p) => p == p.toInt() ? p.toStringAsFixed(0) : p.toStringAsFixed(2);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 4),
        Align(alignment: Alignment.centerRight, child: Text('Total: \$${_formatTotal(totalAmount)}',
          style: typo.headlineSmall.copyWith(color: colors.primary, fontWeight: FontWeight.bold))),
      ]),
    );
  }
}
...
class _StepperConfigurator extends ConsumerWidget {
  const _StepperConfigurator({required this.label, required this.unit, required this.listing});
  final String label;
  final String unit;
  final Listing listing;
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _DateBox(text: formatter.format(startDate), onTap: () async {
            final picked = await showDatePicker(context: context,
              initialDate: startDate.isBefore(DateTime.now()) ? DateTime.now() : startDate,
              firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 365)));
            if (picked != null) ref.read(rentalStartDateProvider.notifier).setDate(DateTime(picked.year, picked.month, picked.day));
          }),
        ])),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 4),
          Row(children: [
            GestureDetector(onTap: () => ref.read(rentalDurationProvider.notifier).decrement(), child: _StepperButton(text: '-')),
            ...
            const SizedBox(width: 8),
            GestureDetector(onTap: () => ref.read(rentalDurationProvider.notifier).increment(), child: _StepperButton(text: '+')),
          ]),
        ])),
      ]),
      const SizedBox(height: 12),
      _TotalRentBanner(periodText: '$totalDays days (${formatter.format(startDate)} to ${formatter.format(calculatedEndDate)})', totalAmount: totalRent),
    ]);
  }
}
