...
class _AdminListingsScreenState extends ConsumerState<AdminListingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Manage Listings', style: typo.headlineSmall.copyWith(fontWeight: FontWeight.w800)),
        backgroundColor: colors.surfaceContainerLowest,
              ...
              children: [
                ...
                DropdownButton<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'delisted', child: Text('Delisted')),
                  ],
          ...
          Expanded(
            child: listingsState.when(
              ...
              data: (listings) {
                var filtered = listings.where((l) {
                  ...
                  if (_searchQuery.isNotEmpty) {
                    ...
                    return (l['title'] ?? '').toString().toLowerCase().contains(q) ||
                        (l['category'] ?? '').toString().toLowerCase().contains(q) ||
                        (l['seller_name'] ?? '').toString().toLowerCase().contains(q);
                  }
                  return true;
                }).toList();
                ...
                if (filtered.isEmpty) {
                  return Center(
                    child: Text('No listings found.', style: typo.bodyLarge.copyWith(color: colors.onSurfaceVariant)),
                  );
                }
                ...
                return SingleChildScrollView(
                  ...
                  child: SingleChildScrollView(
                    child: DataTable(
                      headingRowColor: WidgetStateProperty.all(colors.surfaceContainerLow),
                      columns: const [
                      ...
                      rows: filtered.map((l) {
                        ...
                        return DataRow(cells: [
                          ...
                          DataCell(Text('\$${(l['price'] ?? 0).toStringAsFixed(0)}')),
                          ...
                          DataCell(Text(
                            l['created_at'] != null
                                ? DateFormat('MM/dd/yy').format(DateTime.parse(l['created_at']))
                                : '-',
                          )),
                        ]);
                      }).toList(),
