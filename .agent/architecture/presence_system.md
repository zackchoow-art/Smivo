...
2. **后台时段趋势图**：在 React Admin Dashboard (admin.smivo.io) 查看每小时的用户在线人数走势折线图。
...
ORDER BY date_hour;
```
（这条语句可以直接对接 React 图表库如 Recharts / Chart.js，完美渲染时段走势。）
- **实现位置**：admin/src/features/dashboard/ 模块中，通过 @supabase/supabase-js 调用查询。
