...
final debugDataProvider = FutureProvider.autoDispose<Map<String, dynamic>>((
  ref,
) async {
  final client = ref.watch(supabaseClientProvider);

  // 1. Fetch system configs
  final configsData = await client
      .from('system_configs')
      .select('*')
      .order('config_key');

  // 2. Fetch system settings (Feature Flags)
  final settingsData = await client
      .from('system_settings')
      .select('*')
      .order('key');

  // 3. Fetch system dictionaries and group by dict_type
  final dictsData = await client
  ...
  // 4. Fetch sensitive words and group by category/severity
...
class DebugDataScreen extends ConsumerWidget {
  ...
  Future<void> _toggleSetting(
    ...
    final currentVal = setting['value'];

    // Determine if it's a boolean value (jsonb column)
    final isTrue = currentVal == true || currentVal == 'true';
    final isFalse = currentVal == false || currentVal == 'false';
    if (!isTrue && !isFalse) return;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    try {
      ...
      debugPrint(
        'DebugDataScreen: Provider watched, state: ${asyncData.runtimeType}',
      );
            ...
            child: asyncData.when(
              ...
              data: (data) {
                try {
                  ...
                  final settings = data['system_settings'] as List<dynamic>;
                  final dictsGrouped =
                      data['system_dictionaries'] as Map<String, List<dynamic>>;
                  final wordsSummary =
                      data['sensitive_words_summary'] as Map<String, int>;
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      ...settings.map<Widget>((s) {
                        final settingMap = s as Map;
                        final val = settingMap['value'];
                        final isBool =
                            val == true ||
                            val == false ||
                            val == 'true' ||
                        ...
                        return ListTile(
                          title: Text(
                            settingMap['key']?.toString() ?? 'Unknown',
                            style: typo.bodyLarge,
                          ),
                          subtitle: Text(
                            settingMap['description']?.toString() ?? '',
                                  ...
                                  ? Switch(
                                    ...
                                    onChanged:
                                        (_) => _toggleSetting(
                                          ref,
                                          context,
                                          Map<String, dynamic>.from(settingMap),
                                        ),
                                    activeThumbColor: colors.primary,
                                    activeTrackColor: colors.primary.withAlpha(
                                      100,
                                    ),
                                  )
                                  : null,
                      ...
                      ...configs.map<Widget>((c) {
                        ...
                        return ListTile(
                          title: Text(
                            configMap['config_key']?.toString() ?? 'Unknown',
                            style: typo.bodyLarge,
                          ),
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text(
                                configMap['description']?.toString() ?? '',
                                style: typo.bodySmall,
                              ),
                            ],
                          ),
                      ...
                      ...dictsGrouped.entries.map<Widget>((entry) {
                        return ExpansionTile(
                          ...
                          children:
                              entry.value.map<Widget>((dict) {
                                final dictMap = dict as Map;
                                return ListTile(
                                  title: Text(
                                    dictMap['dict_key']?.toString() ??
                                        dictMap['dict_value']?.toString() ??
                                        'Unknown',
                      ...
                      if (wordsSummary.isEmpty)
                        ...
                        ...wordsSummary.entries.map<Widget>(
                          (e) => ListTile(
