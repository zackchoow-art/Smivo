-- ============================================================
-- Migration 00086: Fix cleanup to include audit logs & all feedbacks
-- ============================================================
-- Issues found:
--   1. user_feedbacks only deleted for school users — orphan
--      feedbacks from users without school_id survive
--   2. admin_audit_logs never deleted by cleanup
--   3. contribution_ledger same issue as user_feedbacks
-- For pre-launch cleanup, these should all be purged completely.
-- ============================================================
...
BEGIN
  ...
  -- ── Pre-launch: purge ALL feedbacks, contributions, and audit logs ──
  -- NOTE: These tables are not school-scoped in schema, so we delete everything
  -- for a clean pre-launch state. This is intentional for test data cleanup.
